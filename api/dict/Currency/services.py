...
from .schemas import CurrencyBase, CurrencyInDB
...
def obj_meta(obj):
    return {
        ...
        "code": {"label": "Код валюты", "value": obj.code},
        "copybook_parameters_ru": {"label": "Параметры прописи - ru", "value": obj.copybook_parameters_ru},
        "copybook_parameters_en": {"label": "Параметры прописи - en", "value": obj.copybook_parameters_en}
    }
...
class CurrencyService:
    ...
    async def update_object(obj_id: int, obj_schema: CurrencyInDB, db: AsyncSession = Depends(get_db)):
        result = await db.execute(select(Currency).where(Currency.id == obj_id))
        obj = result.scalars().one_or_none()
        if obj is None:
            return None
