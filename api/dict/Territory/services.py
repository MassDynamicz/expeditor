...
from .schemas import TerritoryBase, TerritoryInDB
...
class TerritoryService:
    async def get_list(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)):
        result = await db.execute(select(Territory).offset(skip).limit(limit))
        objs = result.scalars().all()
        r_object = [obj_meta(obj) for obj in objs]
        return r_object

    async def get_object(obj_id: int, db: AsyncSession = Depends(get_db)):
        result = await db.execute(select(Territory).where(Territory.id == obj_id))
        obj = result.scalars().one_or_none()
        if obj is None:
            return None
        return obj_meta(obj)

    async def create_object(obj_schema: TerritoryBase, db: AsyncSession = Depends(get_db)):
        new_obj = Territory(**obj_schema.dict())
        db.add(new_obj)
        await db.commit()
    ...
    async def update_object(obj_id: int, obj_schema: TerritoryInDB, db: AsyncSession = Depends(get_db)):
        result = await db.execute(select(Territory).where(Territory.id == obj_id))
        obj = result.scalars().one_or_none()
        if obj is None:
            return None
